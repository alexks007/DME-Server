...
module.exports = function(app) {
  ...
  app.get('/event-list', async(req, res) => {
    try {
      ...
      events.map(event => {
        ...
        if (event.status == 1) event.strStatus = 'Playing';
        else if (event.status == 2) event.strStatus = 'End';
        else event.strStatus = 'Playing';
      })
  ...
  app.get('/my-profile', async(req, res) => {
    try {
      ...
      res.render('my-profile', {
        ...
        mail_id: mailed[0] == 'title="Email' ? '' : mailed[0],
        mail_type: mailed[1] == 'title="Email' ? '': mailed[1],
      })
    } catch(e) {
      // return Promise.reject(e);
    }
  ...
  app.get('/searchdetail', async(req, res) => {
    try {
      ...
      store.system_lefthand = store.system_lefthand ? store.system_lefthand : 'Empty';
      store.swingplate = store.swingplate ? store.swingplate : 'Empty';
      store.parking_status = store.parking_status ? store.swingplate : 'Empty';
  ...
  app.get('/my-swing', async(req, res) => {
    try {
      ...
      if (user.user_setDifficulty == 1) difficulty = 'SemiTua';
      else if (user.user_setDifficulty == 2) difficulty = 'SemiTua';
